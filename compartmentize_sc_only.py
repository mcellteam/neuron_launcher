...
def f_compartmentize_sc_only(context, swc_filepath):
    ...
    # Keep track of names
    name_dict = {}
    for sc_id in mn_section_dict.keys():
        name_dict[sc_id] = 1
    # Make the planes
        ...
        for end_list in sc.planes_sc_brdrs: 
            for plane in end_list:
                if not plane.name in planes_done_list:
                    plane.make_separate_object(context, ob_vert_list, name_dict)
    ...
    for sec in reg_list:
        ...
        print("Making surface object for section: " + str(sec_name))
        ...
        if len(sec_name) == 8 and sec_name[0:3] == 'sc_':
            ...
            # Make a new object out of these faces
            obj_from_faces(context, "sc_%02d_%02d_id_%02d"%(pt_min,pt_max,name_dict[(pt_min,pt_max)]), f_list, ob)
            name_dict[(pt_min,pt_max)] += 1
    ...
    ############################
    ############################
    # Join the objects for ease of use into a single compartment
    ############################
    ############################

    bpy.ops.object.mode_set(mode='OBJECT')

    # Keep track of which sections we've already joined
    sec_joined = []

    obj_list = [item.name for item in context.scene.objects]
    for oname in obj_list:

        # Check that its one of the class that we wish to join
        if len(oname) == 14 and oname[0:3] == "sc_" and oname[-5:-3] == "id":

            if not oname[0:8] in sec_joined:

                # Get the obj
                ob0 = bpy.data.objects[oname]

                print("Joining objects from object: " + str(oname))

                # Deselect all b[ojects
                bpy.ops.object.select_all(action='DESELECT')

                # Select the ones to join
                ob0.select = True
                for ob1 in context.scene.objects:
                    if len(ob1.name) == 14 and ob1.name[0:3] == "sc_" and ob1.name[-5:-3] == "id" and ob1.name[0:8] == oname[0:8]:
                        ob1.select = True
                ...
                # Rename
                ob_list = context.selected_objects
                ob_list[0].name = ob_list[0].name[0:8] + "_compartment"

                # Keep track that this section is joined
                sec_joined.append(oname[0:8])

    print("Finished joining objects")
    ...
    # Get the materials assigned to each section
    ...
    # Assign the materials to the other objects
    for ob0 in context.scene.objects:
        ...
        if len(oname) == 20 and oname[0:3] == "sc_" and oname[-11:] == "compartment":
            ...
            # Check that the material exists in the dictionary (this section might not have a material!)
            if oname[0:8] in mat_dict:

                print("Assigning materials to: " + str(oname))
                ...
                for f in ob0.data.polygons:
                    f.select = True
                bpy.ops.object.mode_set(mode='EDIT')

                # Copy the material
                bpy.ops.object.material_slot_add()
                context.object.material_slots[context.object.material_slots.__len__() - 1].material = mat_dict[oname[0:8]]
